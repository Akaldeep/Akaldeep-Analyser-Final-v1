    ...
    <TooltipProvider>
      <Tooltip delayDuration={0}>
        <TooltipTrigger asChild>
          <button className="ml-1 text-white/20 hover:text-primary transition-colors"><Info className="w-3 h-3" /></button>
        </TooltipTrigger>
        <TooltipContent className="max-w-[220px] p-3 bg-card border-border">
          <p className="text-[9px] font-mono text-primary uppercase tracking-wider mb-1">{def.full}</p>
          <p className="text-[10px] text-white/50 leading-relaxed">{def.def}</p>
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
...
function getBetaCfg(beta: number | null) {
  if (beta === null) return { label: "N/A",        color: "text-white/40",   bg: "bg-white/4",         Icon: Minus };
        ...
        <div className="rounded-lg border border-primary/20 bg-primary/5 px-4 py-3 flex flex-wrap items-center gap-4">
          ...
          <div className="flex flex-wrap gap-4 text-[10px] font-mono text-white/40">
            <span className="flex items-center gap-1.5">
              ...
              Beta computed from <span className="text-white/70">{data.dataPoints?.toLocaleString() ?? "—"} daily price observations</span>
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" style={{ animationDelay: "0.3s" }} />
              Volatility annualised via <span className="text-white/70">√252 scaling</span>
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-blue-400 animate-pulse" style={{ animationDelay: "0.6s" }} />
              Regression against <span className="text-white/70">{data.marketIndex}</span>
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-purple-400 animate-pulse" style={{ animationDelay: "0.9s" }} />
              Currency conversion at <span className="text-white/70">live USD/INR rate</span>
            </span>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div variants={up} className="card-premium">
        <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-white/[0.02]">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-mono text-white/35 uppercase tracking-[0.18em]">Primary Asset Analysis</span>
          </div>
          <a href={data.sourceUrl || `https://finance.yahoo.com/quote/${data.ticker}`} target="_blank" rel="noopener noreferrer"
            className="text-[9px] font-mono text-white/20 hover:text-primary transition-colors flex items-center gap-1 uppercase tracking-wider">
          ...
          <div className="flex flex-col lg:flex-row lg:items-start gap-5">
            <div className="flex-1 space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {data.sector && (
                  <span className="flex items-center gap-1 text-[9px] font-mono text-white/30 bg-white/5 border border-white/8 px-2 py-0.5 rounded">
                ...
                <span className="text-[9px] font-mono text-white/20 bg-white/4 border border-white/6 px-2 py-0.5 rounded">
            ...
            <div className={`flex items-center gap-4 px-5 py-4 rounded-lg border border-white/8 ${betaCfg.bg} shrink-0`}>
              ...
              <div className="space-y-1">
                ...
                <div className="text-[9px] font-mono text-white/25 uppercase tracking-wider">Beta Coefficient</div>
            ...
            return (
              <div key={cat}>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[8px] font-mono text-white/20 uppercase tracking-[0.2em]">{cat}</span>
                  ...
                  {metricsInCat.map(id => {
                    ...
                    return (
                      <div key={id} className="bg-white/[0.03] rounded border border-white/6 px-3 py-2.5 hover:border-primary/20 transition-colors group">
                        <div className="flex items-center text-[8px] font-mono text-white/30 uppercase tracking-wider mb-1.5">
      ...
      <motion.div variants={up} className="card-premium">
        <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-white/[0.02]">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-mono text-white/35 uppercase tracking-[0.18em]">Industry Benchmark</span>
          </div>
          {data.industry && (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-border">
          {[{ label: "Average Beta", v: avgBeta }, { label: "Median Beta", v: medianBeta }].map(({ label, v }) => (
            <div key={label} className="p-8 flex flex-col items-center gap-2">
              <span className="text-[9px] font-mono text-white/25 uppercase tracking-[0.18em]">{label}</span>
              ...
              <span className="text-[8px] font-mono text-white/20">{data.peers.length} peers</span>
            </div>
          ))}
        </div>
          ...
          return (
            <div className="px-5 pb-4 pt-2 border-t border-border">
              <div className="flex justify-between mb-1.5">
                <span className="text-[8px] font-mono text-white/20 uppercase tracking-wider">Peer Beta Distribution</span>
                <span className="text-[8px] font-mono text-white/20">{Math.min(...validBetas).toFixed(2)} — {Math.max(...validBetas).toFixed(2)}</span>
              </div>
      ...
      <motion.div variants={up} className="card-premium">
        <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-white/[0.02]">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-mono text-white/35 uppercase tracking-[0.18em]">Peer Comparables</span>
            <span className="text-[9px] font-mono text-white/20 bg-white/5 px-1.5 py-0.5 rounded">{data.peers.length}</span>
          </div>
          <div className="flex items-center gap-2">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-64 max-h-80 overflow-y-auto">
                {CATEGORY_ORDER.map(cat => {
                  ...
                  return (
                    <div key={cat}>
                      <DropdownMenuLabel className="text-[8px] font-mono uppercase tracking-widest text-white/30">{cat}</DropdownMenuLabel>
                      ...
                      {ids.map(id => (
                        <DropdownMenuCheckboxItem key={id} checked={visible.includes(id)} onCheckedChange={() => toggle(id)} className="text-xs font-mono">
                          ...
                          <span className="ml-2 text-white/30 text-[9px]">— {METRIC_DEFS[id].full}</span>
          ...
          <Table className="premium-table">
            <TableHeader>
              <TableRow className="hover:bg-transparent border-border">
                <TableHead className="w-8 pl-5 py-3 text-[8px] font-mono uppercase tracking-widest text-white/20">#</TableHead>
                <TableHead className="min-w-[180px] py-3 text-[8px] font-mono uppercase tracking-widest text-white/20">Company</TableHead>
                <TableHead className="text-right py-3 text-[8px] font-mono uppercase tracking-widest text-white/20 whitespace-nowrap">
                ...
                {ALL_METRIC_IDS.filter(id => visible.includes(id)).map(id => (
                  <TableHead key={id} className="text-right py-3 text-[8px] font-mono uppercase tracking-widest text-white/20 whitespace-nowrap">
            ...
            <TableBody>
              {data.peers.map((peer, idx) => {
                ...
                return (
                  ...
                  >
                    <TableCell className="pl-5 py-3.5 font-mono text-[9px] text-white/20 tabular-nums">{idx + 1}</TableCell>
                    <TableCell className="py-3.5">
                      <div className="flex flex-col gap-0.5">
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          <span className="font-mono text-[9px] text-white/25 uppercase tracking-wider">{peer.ticker}</span>
                          {peer.industry && (
                    ...
                    {ALL_METRIC_IDS.filter(id => visible.includes(id)).map(id => {
                      ...
                      return (
                        <TableCell key={id} className="text-right py-3.5">
                          <span className="font-mono text-xs text-white/40 tabular-nums whitespace-nowrap">
